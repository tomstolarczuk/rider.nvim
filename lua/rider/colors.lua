...
---     Defaults to RiderConfig.colors.
---   - theme: Use selected theme. Defaults to RiderConfig.theme
...
function M.setup(opts)
	opts = opts or {}
	local override_colors = opts.colors or require("rider").config.colors
	local theme = opts.theme or require("rider")._CURRENT_THEME -- WARN: this fails if called before rider.load()

	if not theme then
		error(
			"rider.colors.setup(): Unable to infer `theme`. Either specify a theme or call this function after ':colorscheme rider'"
		)
	end
	...
	local theme_colors = require("rider.themes")[theme](updated_palette_colors)
