...
local get_compiled_path = function(theme)
    return table.concat({vim.fn.stdpath("state"), "rider", theme .. "_compiled.lua"}, PATH_SEP)
end
...
function M.compile(theme, highlights, termcolors)
    vim.loop.fs_mkdir(vim.fn.stdpath("state") .. PATH_SEP .. "rider", 448)

    local fname = get_compiled_path(theme)
    local file, err = io.open(fname, "wb")
    if not file or err then
        vim.notify("Rider: Error writing " .. fname .. ":\n" .. err, vim.log.levels.ERROR)
        return
    end

    local lines = {
        "require'rider'.compiled = string.dump(function()",
        "local g = vim.g",
    ...
    file:write(require("rider").compiled)
    file:close()
end
